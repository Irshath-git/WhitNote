...
function NoteDialog(props: Props) {
  const { activePage, onShowDeleteConfirm, onSyncDeleteAmount } = props;

  const { isNoteDialogVisible, noteTheme, noteFont, noteIsFavourite } =
  ...
  return (
    <form className={`note-dialog ${noteDialogClasses}`}>
      <div onClick={hideFontAndPalette} className="action-buttons">
        <button onClick={closeNoteDialog} className="close-button">
          <ArrowLeft />
        </button>
        <button
